...
For li_idx = 1 To li_max
	...
	Else
		ll_sorttype = 1
		ll_filesize = (lstr_dirlist[li_idx].db_filesize + 512) / 1024
		If lstr_dirlist[li_idx].db_filesize > 0 And ll_filesize = 0 Then
			ll_filesize = 1
		End If
	End If
...
For ll_row = 1 To ll_max
	...
	If ll_sorttype = 0 Then
		...
		lv_files.SetItem(li_item, 3, "File Folder")
		lv_files.SetItem(li_item, 4, String(ldt_modified, "[shortdate] hh:mm"))
	Else
		// file
		ls_filename  = ids_sorter.GetItemString(ll_row, "filename")
		ldt_modified = ids_sorter.GetItemDateTime(ll_row, "modified")
...
m_main lm_menu
String ls_subkey, ls_server, ls_userid, ls_password, ls_initialdir
...
If lb_result = False Then
	gw_frame.wf_addmsg(gn_ftp.LastErrorMsg)
	MessageBox("Connect Error " + String(gn_ftp.LastErrorNbr), &
					gn_ftp.LastErrorMsg, StopSign!)
	Return False
End If
