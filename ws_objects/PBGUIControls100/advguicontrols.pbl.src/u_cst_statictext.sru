...
public function integer of_createfont (unsignedlong a_hdc)
end prototypes
...
IF #Bold THEN
	inv_win32.SelectObject(ahdc, iul_fontbold)
ELSE
	inv_win32.SelectObject(ahdc, iul_font)
END IF
...
inv_win32.SetTextColor(ahdc, #FontColor)

inv_win32.Drawtext(ahdc, #Text, LEN(#Text), l_Rect, ll_parm)
...
public function integer of_createfont (unsignedlong a_hdc);LogFont lstr_Lf
uLong lul_Font

lstr_Lf.lffacename = #FontName
lstr_Lf.lfweight = 400
lstr_Lf.lfheight = -1 * ( inv_win32.MulDiv(#FontSize, inv_win32.GetDeviceCaps(a_hdc, inv_win32.LOGPIXELSY), 72) )
...
IF NOT #Transparent THEN
	...
	inv_win32.GetClientRect (Handle(THIS), lpRect)
	...
	IF #VGradientFill THEN
		inv_win32.GradientRectangle (hdc, Corner, 4, l_Gradient, 1, GRADIENT_FILL_RECT_V)
	ELSE
		inv_win32.GradientRectangle (hdc, Corner, 4, l_Gradient, 1, GRADIENT_FILL_RECT_H)
	END IF
