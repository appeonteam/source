...
type inv_win32 from n_cst_win32 within u_cst_xplistbargroup
end type
end forward
...
FOR ll_index = 1 TO ll_count
	...
	IF iuo_items[ll_index].il_style <> iuo_parent.OBJECT THEN
		iuo_items[ll_index].of_GetTextSize(iuo_items[ll_index].of_GetText())
	ELSE
		IF LEN(iuo_items[ll_index].is_image) > 0 THEN
			iuo_items[ll_index].iuo_object.Width = THIS.Width - 220
		ELSE	
			iuo_items[ll_index].iuo_object.Width = THIS.Width - 100
		END IF
	END IF
NEXT
...
iuo_parent.Event GroupExpanding(uo_header.of_GetText())
...
iuo_parent.Event GroupCollapsing(uo_header.of_GetText())
...
inv_win32.SetParent(Handle(a_object), Handle(iuo_items[ll_item]))
...
iuo_items[ll_item].Y = uo_header.Height + 20 + ll_height
iuo_items[ll_item].Height = a_object.Height + 4// 100
...
FOR ll_index = 1 TO ll_count
	...
	ELSE
		inv_win32.SetParent(Handle(iuo_items[ll_index]), Handle(iuo_parent.iw_parent))
...
FOR ll_index = 1 TO ll_count
	IF IsValid(iuo_items[ll_index]) THEN
		...
		inv_win32.SetParent(Handle(iuo_items[ll_index]), Handle(iuo_parent.iw_parent))
		inv_win32.SetParent(Handle(iuo_items[ll_index].iuo_object), Handle(iuo_parent.iw_parent))
		iuo_parent.iw_parent.CloseUserObject(iuo_items[ll_index])
	END IF
...
// Revision History
// Date
// 20080121		Allan Cartagenas		Initial version
//
////////////////////////////////////////////////////////////

il_link_idx = al_link_idx
...
type inv_win32 from n_cst_win32 within u_cst_xplistbargroup descriptor "pb_nvo" = "true" 
end type
