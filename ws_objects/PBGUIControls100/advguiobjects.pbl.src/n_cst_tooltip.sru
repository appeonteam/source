...
type inv_win32 from n_cst_win32 within n_cst_tooltip
end type
end forward
...
Constant long TTM_UPDATETIPTEXT	= WM_USER + 57
...
If ab_Status Then 
	inv_win32.ToolTipMsg( hWndTT, TTM_TRACKACTIVATE, 1, ToolInfo )
Else
	inv_win32.ToolTipMsg( hWndTT, TTM_TRACKACTIVATE, 0, ToolInfo )
End If
end subroutine
...
inv_win32.ToolTipMsg( hWndTT, TTM_NEWTOOLRECT, 0, ToolInfo )
...
inv_win32.ToolTipMsg( hWndTT, TTM_UPDATETIPTEXT, 0, ToolInfo )
end subroutine
...
lpszText = inv_win32.LocalAlloc( 0, 255 )
inv_win32.lStrCpy( lpszText, Left( as_TipText, 255 ) )
of_SetTipText( ado_Object, al_uId, lpszText )

inv_win32.LocalFree( lpszText )
end subroutine
...
ToolID++
ToolInfo.lpszText	= inv_win32.LocalAlloc( 0, 120 )
inv_win32.POST LocalFree( ToolInfo.lpszText ) // Free Allocated Memory
inv_win32.lStrCpy( ToolInfo.lpszText, Left( as_tiptext, 120 ) )
...
If inv_win32.ToolTipMsg( hWndTT, TTM_ADDTOOL, 0, ToolInfo ) = 0 Then
...
inv_win32.RelayMsg( hWndTT, TTM_RELAYEVENT, 0, Msg )
...
inv_win32.ToolTipMsg( hWndTT, TTM_DELTOOL, 0, ToolInfo )
...
inv_win32.SendMessageString( hWndTT, TTM_SETTITLEA, ai_Icon, as_Title )
end subroutine
...
//ToolInfo.Rect.Right	= UnitsToPixels( ado_Object.Width, XUnitsToPixels! )
//ToolInfo.Rect.Bottom	= UnitsToPixels( ado_Object.Height, YUnitsToPixels! )
inv_win32.GetClientRect(aul_handle, ToolInfo.Rect)
...
IF inv_win32.InitCommonControlsEx(lstr_iccex) THEN

	hWndTT = 	inv_win32.CreateWindowEx(WS_EX_TOPMOST,TOOLTIPS_CLASS,0, TTF_CENTERTIP, &
...
event destructor;inv_win32.DestroyWindow(hWndTT)
end event

type inv_win32 from n_cst_win32 within n_cst_tooltip descriptor "pb_nvo" = "true" 
end type
