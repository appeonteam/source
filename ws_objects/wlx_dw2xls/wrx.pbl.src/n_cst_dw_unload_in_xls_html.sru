$PBExportHeader$n_cst_dw_unload_in_xls_html.sru
$PBExportComments$выгрузка в xls через html
forward
global type n_cst_dw_unload_in_xls_html from n_cst_dw_unload
end type
end forward

global type n_cst_dw_unload_in_xls_html from n_cst_dw_unload
end type
global n_cst_dw_unload_in_xls_html n_cst_dw_unload_in_xls_html
...
on n_cst_dw_unload_in_xls_html.create
TriggerEvent( this, "constructor" )
end on

on n_cst_dw_unload_in_xls_html.destroy
...
//				CreateTag(BandCells[li_nom_element]) //формировать тег для ячейки с данными
				...
				sTag +=' style="vnd.ms-excel.numberformat:' + istr_band_cells[li_nom_element].s_xls_format + '"' // тип данных в ячейке
				...
				if istr_band_cells[li_nom_element].s_display_value = '' then
					...
					if ( IsNumber(istr_band_cells[li_nom_element].s_real_value) and (istr_band_cells[li_nom_element].s_display_value) <> '-') then
						sTag += of_str_replace(istr_band_cells[li_nom_element].s_real_value,is_separator_decimal,'.')
					else
