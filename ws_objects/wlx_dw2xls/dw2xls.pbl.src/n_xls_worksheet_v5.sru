...
forward prototypes
protected function integer of_reg_format (unsignedinteger ai_row, unsignedinteger ai_col, n_xls_format_v5 anvo_format)
...
protected function integer of_reg_format (unsignedinteger ai_row, unsignedinteger ai_col, n_xls_format_v5 anvo_format);uint li_xf = 15
ulong ll_key
...
li_xf = of_reg_format(ai_row, ai_col, lnvo_format)
...
public function integer of_write (unsignedinteger ai_row, unsignedinteger ai_col, double adb_num);n_xls_format_v5 lnvo_format
setnull(lnvo_format)
return of_write(ai_row, ai_col, adb_num, lnvo_format)
end function

public function integer of_write (unsignedinteger ai_row, unsignedinteger ai_col, string as_str);n_xls_format_v5 lnvo_format
setnull(lnvo_format)
return of_write(ai_row, ai_col, as_str, lnvo_format)
end function

public function integer of_write (unsignedinteger ai_row, unsignedinteger ai_col, time at_time);n_xls_format_v5 lnvo_format
setnull(lnvo_format)
return of_write(ai_row, ai_col, at_time, lnvo_format)
end function

public function integer of_write (unsignedinteger ai_row, unsignedinteger ai_col, datetime adt_datetime);n_xls_format_v5 lnvo_format
setnull(lnvo_format)
return of_write(ai_row, ai_col, adt_datetime, lnvo_format)
end function

public function integer of_write (unsignedinteger ai_row, unsignedinteger ai_col, date ad_date);n_xls_format_v5 lnvo_format
setnull(lnvo_format)
return of_write(ai_row, ai_col, ad_date, lnvo_format)
end function

public function integer of_write (unsignedinteger ai_row, unsignedinteger ai_col);n_xls_format_v5 lnvo_format
setnull(lnvo_format)
...
public function integer of_set_row (unsignedinteger ai_row, double ad_height, n_xls_format_v5 anvo_format, boolean ab_hidden);integer li_ret = 1
uint li_record = 520
uint li_length = 16
uint li_colmic
uint li_colmac
...
public function integer of_merge_write (unsignedinteger ai_first_row, unsignedinteger ai_first_col, unsignedinteger ai_last_row, unsignedinteger ai_last_col, string as_str, n_xls_format anvo_format);Integer li_ret = 1
UInt li_rwfirst
UInt li_colfirst
UInt li_rwlast
UInt li_collast
...
If Len(as_header) >= 255 Then
	MessageBox("错误", "头注字符必须少于255个字符", stopsign!)
	li_ret = -1
End If
If li_ret = 1 Then
	is_header = as_header
...
If Len(as_footer) >= 255 Then
	MessageBox("错误", "脚注字符少于255个字符!", stopsign!)
	li_ret = -1
End If
...
If ai_scale < 10 OR ai_scale > 400 Then
	MessageBox("错误", "缩放比例范围是从 : 10-400", stopsign!)
	li_ret = -1
End If
If li_ret = 1 Then
	ii_zoom = ai_scale
...
If ai_scale < 10 OR ai_scale > 400 Then
	MessageBox("错误", "打印缩放比例是从 : 10-400", stopsign!)
	li_ret = -1
End If
If li_ret = 1 Then
	ib_fit_page = FALSE
...
public function integer of_write (unsignedinteger ai_row, unsignedinteger ai_col, string as_str, n_xls_format anvo_format);integer li_ret = 1
uint li_record = 516
uint li_length
uint li_xf
uint li_strlen
...
If li_ret = 1 Then
	If Len(lb_data) <= 54 Then
		li_ret = -1
		MessageBox("错误", as_bitmap_filename + " 不包含完整信息", stopsign!)
	End If
End If
If li_ret = 1 Then
	If String(BlobMid(lb_data, 1, 2)) <> "BM" Then
		li_ret = -1
		MessageBox("错误", as_bitmap_filename + " 不能显示一个非法位图!", stopsign!)
	End If
End If
If li_ret = 1 Then
...
If li_ret = 1 Then
	...
	If li_bitcount <> 24 Then
		li_ret = -1
		MessageBox("错误", as_bitmap_filename + " 不是24bit真彩位图BITMAP", stopsign!)
	End If
End If
If li_ret = 1 Then
	If li_planes <> 1 Then
...
If li_ret = 1 Then
	...
	If ll_compression <> 0 Then
		li_ret = -1
		MessageBox("错误", as_bitmap_filename + ": 不提供压缩的位图", stopsign!)
	End If
End If
If li_ret = 1 Then
